...
- 无装备、无随机稀有度的宠物初始白卡
- `PAW//CORE` 原创高端收藏卡：统一 5:7 模板、SVG 切角金属框和五档材质主题
- 普通、稀有、史诗、传说、神话五种稀有度拥有不同轮廓层数、纹理、反光和粒子强度
- ATK / DEF / SPD / VIT / TEC / LUK 六项卡面属性，以及定位、元素、进化、阵营和流派信息
- 卡组列表、卡牌详情、对战选人共用同一个数据驱动组件，并支持锁定、受伤、升级和开卡状态
...
收藏卡设计规格见 [`docs/collectible-card-system.md`](docs/collectible-card-system.md)，主题配置位于 `src/cardSystem.ts`，可复用卡组件位于 `src/CollectiblePetCard.tsx`。照片接口同时返回透明 WebP 立绘层，卡框和所有文字均由客户端实时渲染。
